...
public:
  ...
  void set_updater(rmf_fleet_adapter::agv::RobotUpdateHandlePtr updater)
  {
    ...
    // A map ontains different executors for different actions
    auto executor_map = std::make_shared<std::unordered_map<std::string,
      ActionExecutor>>();

    // std::unordered_map<std::string, ActionExecutor> executor_map;

    // Set the teleop action_executor for the robot
    const auto teleop_executor =
      [w = weak_from_this()](
      const std::string&,
      const nlohmann::json&,
    ...
    executor_map->emplace("hello", hello_executor);

    const ActionExecutor null_executor =
      [](
      const std::string&,
      const nlohmann::json&,
      ActionExecution execution)
    ...
    executor_map->emplace("null", null_executor);

    const ActionExecutor dispatching_executor = [executor_map](
    ...
    _travel_info.updater->set_action_executor(dispatching_executor);
  }
...
struct Connections : public std::enable_shared_from_this<Connections>
{
  ...
  /// The publisher for sending out message for hello task
  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr
    hello_pub;
  ...
  const rmf_fleet_adapter::agv::AdapterPtr& adapter)
{
  ...
  connections->fleet->add_performable_action(
    ...
    // Configure this fleet to perform any hello action
  connections->fleet->add_performable_action(
    "hello",
    consider_hello);
